...
public class TestFluentApiCalcContext {
    ...
    public void Const_SomeFieldInitialized_DefaultValuesInUninitalizedFields() {
        ...
        CalcInDifferentWays(expr, context, new ContractOutputModel { Id = 321 });
    }
    ...
    public void FullConstInitialization() {
        ...
        var context = new ContextModel1(imodel: input);
        var expectedContext = new ContextModel1(context.IntRVal, (UserInputModel)input.Clone()) { OModel = expected };
        CalcInDifferentWays("omodel = {id = 42; items = ['vasa','kate']; price = 42.1; taxes = 42.2}", context,
            expectedContext);
    }
    ...
    public void TestImplementedModels() {
        ...
        var calculator = new FunnyCalculatorBuilder().BuildForCalc<bool>(inheritedModel.GetType());
